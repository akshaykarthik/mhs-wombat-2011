...
function menu:update(dt)
    ...
    if vars.credits.status == "clicked" then
        gamestate.switch(states["credits"])
    end

    --[[
    if vars.single.status == "clicked" then
        gamestate.switch(states["single_menu"])
    end

    if vars.multi.status == "clicked" then
        gamestate.switch(states["multi_menu"])
    end
    ]]
end
