	...
	void process(String rec) {
		...
		while (s.hasNext()) {
			...
			if (s.next().equals("LID")) {
				int id = s.nextInt();
				String objectID = s.next(); // String for loading image +
											// wireframe
				int cx = s.nextInt();
				int cy = s.nextInt();
				LevelPiece p = new LevelPiece();// XXXXX--Must keep track of all
												// pieces to assign ID's. How
												// would we do this? Global
												// Variable?
				p.translateTo(cx, cy);
			}
